...
def get_rel_scores(entity_emb, entity_scores, num_labels, config, dropout, num_predicted_entities):
  ...
  if config['rel_prop']:
    ...
    if config['sigmoid']:
      ...
      flat_rel_scores = tf.concat([dummy_scores, flat_rel_scores], 1)
      max_flat_rel_scores = tf.reduce_max(flat_rel_scores, -1)
      flat_rel_scores_mask = max_flat_rel_scores > 0
    with tf.variable_scope("rel_W"):
      entity_emb_size = util.shape(entity_emb, -1)
      if config['sigmoid']:
        relation_transition = util.projection(tf.sigmoid(flat_rel_scores), entity_emb_size)
      else:
      ...
      if config['sigmoid']:
        flat_rel_scores_mask = tf.reshape(flat_rel_scores_mask, [-1])
        tranformed_embeddings = tf.multiply(tranformed_embeddings, tf.to_float(flat_rel_scores_mask)) #[num_sents * num_ents * num_ents, entity_emb_size]
  ...
  # if config['rel_pair_prop']:
  #   flat_rel_scores = tf.reshape(rel_scores, [num_sentences * num_entities* num_entities, num_labels - 1])
  #   with tf.variable_scope("rel_W"):
  #     pair_emb_size = util.shape(flat_pair_emb, -1)
  #     S = util.projection(flat_rel_scores, pair_emb_size)
  #     rel_mask = tf.reshape(rel_mask, [-1])
  #     S  = tf.multiply(tf.transpose(S * flat_pair_emb), tf.to_float(rel_mask)) #[entity_emb_size, num_sents * num_ents * num_ents]
  #     S = tf.transpose(S)
  #     S = tf.reshape(S, [num_sentences, num_entities, num_entities, pair_emb_size]) #[num_sents, num_ents, num_ents, entity_emb_size] 
  #     S  = tf.reduce_sum(S, 2) #[num_sents, num_ents,entity_emb_size]
  #     S = tf.reshape(S, [num_sentences * num_entities, pair_emb_size])
  #     ####TODO####
  #     flat_rel_scores = tf.reshape(entity_emb, [num_sentences * num_entities, pair_emb_size])
  #     with tf.variable_scope("f"):
  #       f = tf.sigmoid(util.projection(tf.concat([S, entity_emb], 1), entity_emb_size)) # [num_sents * num_ents, entity_emb_size]
  #       entity_emb = f * S + (1 - f) * entity_emb # [num_sents * num_ents, entity_emb_size]
  #     entity_emb = tf.reshape(entity_emb, [num_sentences, num_entities, entity_emb_size])
