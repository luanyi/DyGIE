...
def bilinear(input1, input2, num_hidden_layers):
  if len(input1.get_shape()) > 2:
    current_input1 = tf.reshape(input1, [-1, shape(input1, -1)])
    current_input2 = tf.reshape(input2, [-1, shape(input2, -1)])
    current_input2 = tf.expand_dims(current_input2, -1)
    #[num_samples, emb2, 1]
  else:
    current_input1 = input1
    current_input2 = input2
  bilinear_weights = tf.get_variable("bilinear_weights",[shape(current_input1, 1), num_hidden_layers, shape(current_input2, 1)])
  current_outputs = tf.tensordot(current_input1,bilinear_weights,[[-1],[0]]) # [num_samples, num_hidden_layers, emb2]
  current_outputs = tf.matmul(current_outputs, current_input2) # [num_samples, num_hidden_layers, 1]
  current_outputs = tf.squeeze(current_outputs, -1)
  # current_outputs = tf.reshape(current_outputs, [shape(input1, 0), shape(input1, 1), shape(input1, 2), num_hidden_layers]) # [num_sentences, num_ents, num_ents, num_hidden_layers]
