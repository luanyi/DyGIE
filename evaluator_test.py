...
if __name__ == "__main__":
  ...
  if config["lm_path"] and "lm_path_dev" in config and config["lm_path_dev"]:
    config["lm_path"] = config["lm_path_test"]
    config["eval_path"] = config["eval_path_test"]
  ...
  # if config["lm_path"] and "lm_path_test" in config and config["lm_path_test"]:
  #   test_config = copy.deepcopy(config)
  #   test_config = copy.deepcopy(config)
  #   test_config["lm_path"] = config["lm_path_test"]
  #   test_data = LSGNData(test_config)
  #   test_model = SRLModel(test_data, test_config)
  #   test_evaluator = LSGNEvaluator(test_config)
  ...
  with tf.Session() as session:
    while True:
      ...
      if ckpt and ckpt.model_checkpoint_path and ckpt.model_checkpoint_path not in evaluated_checkpoints:
        ...
        # if config["lm_path"] and "lm_path_test" in config and config["lm_path_test"]:
        #   print 'Test start'
        #   test_summary, test_f1, test_task_to_f1 = test_evaluator.evaluate(
        #     session, test_data, test_model.predictions, test_model.loss)
        ...
        if f1 > max_f1:
          ...
          copy_checkpoint(tmp_checkpoint_path, os.path.join(log_dir, "model.test_max.ckpt"))
