...
class SRLModel(object):
  # def __init__(self, lsgn_data, coref_data, config, coref_config):
  def __init__(self, lsgn_data, config):
    self.config = config
    # self.coref_config = coref_config
    self.data = lsgn_data 
    # self.coref_data = coref_data
    ...
    #   self.coref_predictions, self.coref_loss = self.get_predictions_and_loss(self.coref_data.input_dict, self.coref_data.labels_dict, self.coref_config)
    # self.loss = self.loss + self.coref_loss
  ...
  def get_predictions_and_loss(self, inputs, labels, config):
    ...
    if config["coref_depth"]:
      ...
      # since mention_doc_ids (e.g. [4,4,4,6,6,7,7,7]) starts from the beginning of doc_id (not start from 0) num_segments is bigger than real number of segments. The redundent dimention is padded with 0 e.g. [0,0,0,0,110,99, 88] -> length = 7
      k_Print = tf.Print(k,
      ...
      for i in range(self.config["coref_depth"]):
        ...
        # ###1127###
        # top_antecedent_mask = tf.log(tf.to_float(top_antecedent_scores > 0))
        # top_antecedent_weights = tf.nn.softmax(tf.concat([dummy_scores, top_antecedent_scores + top_antecedent_mask], 1)) # [k, c + 1]
        # top_antecedent_emb = tf.concat([tf.expand_dims(mention_emb, 1), top_antecedent_emb], 1) # [k, c + 1, emb]
        # attended_span_emb = tf.reduce_sum(tf.expand_dims(top_antecedent_weights, 2) * top_antecedent_emb, 1) # [k, emb]
        # #########
      ...
      # update candidate_span_emb with coreference propagated emb
      # candidate_span_emb = assign_value(candidate_span_emb, top_mention_indices, top_span_emb)
...
########    
    ...
    if config["relation_weight"] > 0:
      if config['entity_beam']:
        ...
        # predict_dict.update({
        #   "candidate_entity_scores": ner_scores,
        # })
      else:
    ...
    if config["relation_weight"] > 0:
      ...
      #   rel_scores = get_rel_nonzero_scores(
